...
import { UpdatePelajaranDto, usePelajaranControllerFindOneQuery, usePelajaranControllerUpdateMutation } from '@/lib/redux/services/api/endpoints/ApiEiM3';
...
export default function EditPelajaranPage({ params }: { params: { id: string } }) {
    ...
    const [formData, setFormData] = useState<UpdatePelajaranDto>({
        jenjang_kelas: 1,
        asal_sekolah: '',
        nama_pelajaran: '',
    });
    ...
    // Update formData ketika data pelajaran berhasil di-load
    useEffect(() => {
        if (getPelajaran) {
            setFormData({
                jenjang_kelas: getPelajaran.pelajaran.jenjang_kelas,
                asal_sekolah: getPelajaran.pelajaran.asal_sekolah,
                nama_pelajaran: getPelajaran.pelajaran.nama_pelajaran,
            });
        }
    }, [getPelajaran]);
    ...
    <>
        {getPelajaran ? 
            ...
            {/* Submit Button */}
            <div>
                <Button type="submit" disabled={isLoadingUpdatePelajaran}>
                    {isLoadingUpdatePelajaran ?
                    <Spinner />
                    :
                    "Simpan Pelajaran"
                    }
